...
MODULE_DESCRIPTION("A simple Linux char driver for writing");  ///< The description -- see modinfo
MODULE_VERSION("0.1");            ///< A version number to inform users

static int    majorNumber;                  ///< Stores the device number -- determined automatically
char   message[BUFFER_LEN] = {0};           ///< Memory for the string that is passed from userspace
...
static int __init ebbchar_init(void){
   ...
   if (majorNumber<0){
      printk(KERN_ALERT "WriteEbbChar failed to register a major number\n");
      return majorNumber;
   }
   printk(KERN_INFO "WriteEbbChar: registered correctly with major number %d\n", majorNumber);
   ...
   printk(KERN_INFO "WriteEbbChar: device class registered correctly\n");
...
static void __exit ebbchar_exit(void){
   ...
   printk(KERN_INFO "writeEbbChar: Goodbye from the LKM!\n");
}
...
static int dev_open(struct inode *inodep, struct file *filep){
   numberOpens++;
   printk(KERN_INFO "WrtieEbbChar: Device has been opened %d time(s)\n", numberOpens);
   return 0;
}
