...
MODULE_DESCRIPTION("A simple Linux char driver for reading");  ///< The description -- see modinfo
MODULE_VERSION("0.1");            ///< A version number to inform users

static int    majorNumber;                  ///< Stores the device number -- determined automatically

extern char   message[BUFFER_LEN];           ///< Memory for the string that is passed from userspace
extern short  size_of_message;              ///< Used to remember the size of the string stored
...
static int __init ebbchar_init(void){
   ...
   if (majorNumber<0){
      printk(KERN_ALERT "ReadEbbChar failed to register a major number\n");
      return majorNumber;
   }
   printk(KERN_INFO "ReadEbbChar: registered correctly with major number %d\n", majorNumber);
   ...
   printk(KERN_INFO "ReadEbbChar: device class registered correctly\n");
   ...
   printk(KERN_INFO "ReadEbbChar: device class created correctly\n"); // Made it! device was initialized
   return 0;
}
...
static void __exit ebbchar_exit(void){
   ...
   printk(KERN_INFO "ReadEbbChar: Goodbye from the LKM!\n");
}
...
static int dev_open(struct inode *inodep, struct file *filep){
   numberOpens++;
   printk(KERN_INFO "ReadEbbChar: Device has been opened %d time(s)\n", numberOpens);
   return 0;
}
...
static int dev_release(struct inode *inodep, struct file *filep){
   printk(KERN_INFO "ReadEbbChar: Device successfully closed\n");
   return 0;
}
